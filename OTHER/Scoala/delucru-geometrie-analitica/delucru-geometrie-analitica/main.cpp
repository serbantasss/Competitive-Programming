...
pair<POINT,POINT> pri;
int n;
POLIGON P;
double strip_closest(POLIGON strip,double d)
{
    double mm=d;
    sort(strip.p,strip.p+strip.n,cmpy);
    for(int i=0;i<strip.n;++i)
        for(int j=i+1;i<strip.n && (strip.p[j].y-strip.p[j].y)<d;++i)
            if(dist(strip.p[i],strip.p[j])<mm)
                pri.first=strip.p[i],pri.second=strip.p[j],mm=dist(strip.p[i],strip.p[j]);
    return mm;
}
double closest_two_points(int st,int dr)
{
    double mm=INF;
    if(dr-st<=3)
    {
        for(int i=st;i<dr;++i)
            for(int j=i+1;j<dr;++j)
                if(dist(P.p[i],P.p[j])<mm)
                {
                    mm=dist(P.p[i],P.p[j]);
                    if(mm<dist(pri.first,pri.second))
                        pri.first=P.p[i],pri.second=P.p[j];
                }
        return mm;
    }
    int mid=(dr-st)/2;
    //P.p[mid] pct din mij;
    double dst=closest_two_points(st+0,mid);
    double ddr=closest_two_points(mid+1,dr+0);
    if(ddr<dst)
        mm=ddr;
    else
        mm=dst;
    ...
    strip.n=0;
    for(int i=st;i<dr;++i)
        if(fabs(P.p[i].x-P.p[mid].x)<mm)
            strip.p[strip.n++]=P.p[i];
    double d=strip_closest(strip,mm);
    if(d<mm)
        return d;
    return mm;
}
...
//    freopen("1.out","w",stdout);
    ...
    n=P.n;
    double d=closest_two_points(0, n);
    printf("%lf",d);*/
    POINT a,b,c;
